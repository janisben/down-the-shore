...
export default async function handler(req, res) {
  ...
  const { token, signature, initials } = req.body || {};

  if (!token || !signature) {
    return res.status(400).json({
      error: "Lease token and signature are required"
    });
  }
  ...
  const { data: requiredSigners, error: requiredError } =
    await supabase
      .from("lease_signers")
      .select("id,signer_role,is_required,signed_at")
  ...
  const guestSideCompleteBefore =
    (requiredSigners || [])
      .filter(item => item.signer_role !== "owner")
      .every(item => Boolean(item.signed_at));

  if (signer.signer_role === "owner") {
    if (!guestSideCompleteBefore) {
      return res.status(409).json({
        error: "The required guest signatures are not complete yet."
      });
    }
    ...
    const { data: payments, error: paymentError } =
      await supabase
        .from("payments")
        .select("amount")
        .eq("reservation_id", lease.reservation_id);
    ...
    if (paid <= 0) {
      return res.status(409).json({
        error: "A payment must be received before the owner signs."
      });
    }
  }
  ...
  const { error: updateError } =
    await supabase
      .from("lease_signers")
      .update({
        signature_text: String(signature).trim(),
  ...
  const initialRows =
    Object.entries(initials || {})
      ...
      .map(([sectionKey, value]) => ({
        ...
        initials: String(value).trim(),
        initialed_at: signedAt
      }));
  ...
  if (initialRows.length) {
    const { error: initialError } =
      await supabase
        .from("lease_initials")
        .upsert(initialRows, {
          onConflict: "signer_id,section_key"
        });
